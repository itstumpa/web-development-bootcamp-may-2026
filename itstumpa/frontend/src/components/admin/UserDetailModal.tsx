    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-sm bg-[#1E2530] border border-[#334155] rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-[#334155]">
          <h2 className="text-[#F1F5F9] font-semibold text-sm">User Details</h2>
        ...
        {isLoading ? (
          ...
          <div className="px-5 py-5 flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 bg-[#0F1419] rounded-xl p-4">
              {[
                ...
                { label: "Status", value: user.isSuspended ? "Suspended" : user.isOnline ? "Online" : "Offline" },
                ...
                <div key={item.label} className="flex items-center justify-between">
                  ...
                  <span className="text-[#F1F5F9] text-xs font-medium">{item.value}</span>
                </div>
              ))}
            </div>
          ...
          <p className="text-[#94A3B8] text-sm text-center py-12">Failed to load user.</p>
        )}
      </div>
    </div>
  );
